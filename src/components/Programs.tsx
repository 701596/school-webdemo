...
const ProgramCard: React.FC<ProgramCardProps> = ({ 
  icon, level, age, description, highlights, imageUrl
}) => {
  return (
    <div className="bg-white rounded-[32px] overflow-hidden flex flex-col justify-between h-full soft-card-shadow hover:translate-y-[-4px] transition-transform duration-300 border-0">
      <div>
        ...
        <div className="p-6 sm:p-8 pt-5">
          ...
          <div className="flex items-center justify-between mb-5">
            <div className="w-11 h-11 rounded-xl bg-brand-sage/50 flex items-center justify-center text-brand-forest shadow-sm">
              {icon}
            </div>
            <span className="px-3.5 py-1.5 rounded-full bg-brand-sand text-brand-forest text-[10px] font-bold uppercase tracking-wider">
              {age}
            </span>
          </div>
          ...
          <p className="text-sm text-brand-forest/80 mb-6 leading-relaxed font-medium">
            {description}
          </p>
      ...
      <div className="px-6 sm:px-8 pb-8">
        <div className="pt-5 border-t border-brand-sage/30">
          <a 
            href="#contact"
            className="inline-flex items-center gap-1.5 bg-brand-forest hover:bg-brand-forest/95 text-white font-bold text-xs px-6 py-3 rounded-full transition-all duration-300 hover:scale-[1.02] shadow-sm w-fit"
          >
...
const Programs: React.FC = () => {
  const programData = [
    ...
    {
      icon: <GraduationCap className="w-5 h-5 text-brand-forest" />,
      level: "LKG / UKG",
      age: "4 - 5 Years",
  ...
  return (
    <section id="programs" className="py-20 bg-brand-sage/60 relative border-b border-brand-sage/30">
      <div className="max-w-7xl mx-auto px-6 md:px-12">
        ...
        <div className="flex items-center justify-center gap-3 mt-12">
          <button className="w-10 h-10 rounded-full bg-white text-brand-forest hover:bg-brand-sand border border-brand-sage/40 flex items-center justify-center transition-all shadow-sm active:scale-95 cursor-pointer">
            <span>&larr;</span>
          </button>
          <button className="w-10 h-10 rounded-full bg-white text-brand-forest hover:bg-brand-sand border border-brand-sage/40 flex items-center justify-center transition-all shadow-sm active:scale-95 cursor-pointer">
            <span>&rarr;</span>
